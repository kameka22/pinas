export default {
	...
	hardwarePower: {
		...
		governorUnavailable: 'No CPU frequency governor on this hardware (typical in a virtual machine): the hypervisor manages the CPU.',
	...
	onboarding: {
		...
		welcomeScreen: {
			...
			thankYou: 'Thank you for installing PiNAS.',
	...
	storageManager: {
		...
		raidTypes: {
			...
			btrfsRaid0: 'Btrfs RAID0',
			btrfsRaid1: 'Btrfs RAID1',
			btrfsRaid5: 'Btrfs RAID5',
			btrfsRaid6: 'Btrfs RAID6',
			...
			raid6Desc: 'Double parity. Survives 2 simultaneous disk failures.',
			raid10Desc: 'Striping + Mirroring. High performance, survives failures.',
			btrfsSingleDesc: 'Btrfs filesystem, no redundancy.',
			btrfsRaid0Desc: 'Btrfs striping. Maximum capacity, no redundancy.',
			btrfsRaid1Desc: 'Btrfs mirroring. Survives 1 disk failure.',
			btrfsRaid5Desc: 'Btrfs RAID5 with parity. Survives 1 disk failure.',
			btrfsRaid6Desc: 'Btrfs RAID6 double parity. Survives 2 disk failures.',
			btrfsRaid10: 'Btrfs RAID10',
			btrfsRaid10Desc: 'Btrfs Striping + Mirroring. High performance with redundancy.'
		},
		modals: {
			createPool: {
			...
			smartInfo: {
				...
				unsupported: 'S.M.A.R.T. is not available for this disk (virtual disk or USB bridge without SMART pass-through).'
			},
			editPool: {
				title: 'Edit Pool',
				poolName: 'Pool Name',
		...
		power: {
			...
			spindown: 'Spindown Timer',
			writeCache: 'Write Cache',
			writeCacheWarning: 'Disabling write cache reduces performance but improves data safety',
			notSupported: 'Power management not supported for this device type'
		},
		snapshots: {
			title: 'Snapshots',
			...
			btrfsOnly: 'Snapshots are only available for btrfs volumes',
		...
		grow: {
			addDisks: 'Add Disks',
			currentDisks: 'disk(s)',
			selectNewDisks: 'Select new disks',
			noCandidates: 'No available disks',
			expanding: 'Expanding...',
			raidConstraint: 'RAID 10 requires an even number of new disks',
			basicNoGrow: 'Cannot add disks to a basic pool',
			wipeDevices: 'Wipe new disks before adding',
			startExpand: 'Start Expansion',
			warning: 'Expanding a pool cannot be reversed.'
		},
		errors: {
	...
	fileService: {
		...
		nfs: {
			description: 'Network File System for Linux, macOS and media players',
			exports: 'Exports',
			exportsCount: '{n} active exports',
			addExport: 'Add export',
			exportCreated: 'Export created',
			noExports: 'No NFS export yet',
			name: 'Name',
			path: 'Folder',
			clients: 'Allowed clients',
			clientsHint: 'IP, CIDR range or hostname, separated by spaces. * allows everyone on the LAN.',
			readOnly: 'Read only',
			sync: 'Synchronous writes (safer)',
			squash: 'Root mapping',
			notAvailable: 'The NFS server (nfsd) is not part of this image. Rebuild the image with the pinas-nfs-server package.',
			mountHint: 'Mount from a client:'
		},
		cups: {
	...
	shareManager: {
		...
		timeMachineDesc: 'Enable macOS Time Machine backup to this share',
		smbEncrypt: 'Encryption',
		encryptOff: 'Off',
		encryptDesired: 'If supported',
		encryptRequired: 'Required',
		hostsAllow: 'Allowed Hosts',
		hostsDeny: 'Denied Hosts',
		hostsHint: 'Space-separated IPs or subnets (e.g. 192.168.1.0/24)',
		auditLogging: 'Audit Logging',
		auditLoggingDesc: 'Log file operations (create, rename, delete)',
		vetoFiles: 'Veto Files',
		vetoFilesHint: 'Samba veto pattern (e.g. /._*/.DS_Store/)',
		extraOptions: 'Extra Options',
		extraOptionsHint: 'Raw Samba directives (one per line, advanced)',
