<script lang="ts">
	...
	async function toggleService(serviceId: string) {
		...
		try {
			...
			await loadDisplayStatus();
			// If we just activated Kodi, load its data
	...
	async function loadKodiStatus() {
		try {
			kodiStatus = await api.get<KodiStatus>('/kodi/status');
		} catch (e) {
	...
	async function loadKodiInfo() {
		try {
			kodiInfo = await api.get<KodiInfo>('/kodi/info');
		} catch (e) {
	...
	async function setVolume(vol: number) {
		...
		if (kodiStatus) kodiStatus.volume = vol;
	}

	async function gotoPrevious() {
		await api.post('/kodi/playback/previous');
	}

	async function gotoNext() {
		await api.post('/kodi/playback/next');
	}
	...
	$: kodiService = displayStatus?.available_services?.find((s) => s.id === 'kodi');
</script>
...
<div class="display-app">
	...
	<header class="app-header">
		...
		<div class="header-right">
			{#if kodiActive && kodiInfo}
				<span class="version-badge">Kodi v{kodiInfo.version}</span>
			{/if}
		</div>
	</header>
		...
		<div class="service-list">
			{#if displayStatus}
				{#each displayStatus.available_services as service}
					<div class="service-card" class:active={displayStatus.active_service === service.id}>
						<div class="service-info">
							<div class="service-icon">
								{#if service.id === 'kodi'}
									<Icon icon="simple-icons:kodi" />
								{:else}
							...
							<div class="service-status">
								<span class="status-badge" class:running={service.running}>
									{service.running ? $t.kodi.status.connected : $t.kodi.status.disconnected}
								</span>
							</div>
						</div>
						<label class="toggle">
							<input
								type="checkbox"
								checked={displayStatus.active_service === service.id}
								disabled={switching}
						...
						{#if switching}
							<span class="switching-label">
								{displayStatus.active_service === service.id ? $t.display.deactivating : $t.display.activating}
							</span>
						{/if}
					</div>
				{/each}
	...
	<main class="content">
			...
			<div class="tab-content">
				{#if activeTab === 'remote'}
					...
					<div class="remote-panel">
						{#if kodiStatus?.playing}
							<div class="now-playing">
								<Icon icon="mdi:play-circle" class="w-5 h-5" />
								<span>{kodiStatus.current_title || $t.kodi.remote.nowPlaying}</span>
							</div>
						{/if}
		...
		{:else}
			<!-- No active service — Splash info -->
			<div class="splash-info">
				<div class="splash-card">
					<div class="splash-icon">
						<Icon icon="mdi:monitor-screenshot" class="w-16 h-16" />
					</div>
					<h2>{$t.display.noActiveService}</h2>
					<p class="splash-desc">{$t.display.splashActive}</p>
					<div class="splash-url">
						<span class="url-label">{$t.display.splashDescription}</span>
