...
use crate::services::user::{
    ...
    get_user_by_id, get_user_by_id as get_user_for_samba, list_users as list_users_service,
    update_user as update_user_service, UserError, UserUpdate,
};
use crate::AppState;
...
async fn create_user(
    ...
    {
        Ok(user) => {
            // Sync user to Samba (non-blocking on failure)
            ...
            if let Err(e) = share_svc.sync_samba_user(&payload.username, &payload.password).await {
                tracing::warn!("Failed to sync Samba user '{}': {}", payload.username, e);
            }
...
async fn delete_user(
    ...
    // Get username before deletion for Samba cleanup
    ...
    match delete_user_with_home(&state.db, &id, &admin.id, &home_service).await {
        Ok(()) => {
            // Remove user from Samba (non-blocking on failure)
            if let Some(username) = username {
                let share_svc = ShareService::new(state.db.clone());
                if let Err(e) = share_svc.remove_samba_user(&username).await {
                    tracing::warn!("Failed to remove Samba user '{}': {}", username, e);
                }
            }
...
async fn change_user_password(
    ...
    match change_password(&state.db, &id, &payload.password).await {
        Ok(()) => {
            // Sync new password to Samba (non-blocking on failure)
            if let Ok(Some(user)) = get_user_by_id(&state.db, &id).await {
                ...
                if let Err(e) = share_svc.sync_samba_user(&user.username, &payload.password).await {
                    tracing::warn!("Failed to sync Samba password for '{}': {}", user.username, e);
                }
            }
            StatusCode::NO_CONTENT.into_response()
        }
